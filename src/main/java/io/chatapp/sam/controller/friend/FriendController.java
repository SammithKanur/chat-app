...
public class FriendController implements Controller {
    ...
    public ResponseEntity<?> request(String subtype, String message) throws Exception {
        ...
        switch(subtype) {
            ...
            case("chat-write"):
                ...
                break;
            case("update-calling"):
                result = new ResponseEntity<>(updateCalling(friendDto), textHttpHeader, HttpStatus.OK);
        }
        return result;
    }
    ...
    private String createFriendRequest(FriendDto friendDto) throws Exception {
        friendsService.insertFriend(Builder.friendsBuilder(friendDto.getUser(), friendDto.getConnection(), 2, 0));
        friendsService.insertFriend(Builder.friendsBuilder(friendDto.getConnection(), friendDto.getUser(), 1, 0));
        return "success";
    }
    private String updateFriend(FriendDto friendDto) throws Exception {
        ...
        friendsService.updateStatus(Builder.friendsBuilder(friendDto.getUser(), friendDto.getConnection(), 3, 0));
        friendsService.updateStatus(Builder.friendsBuilder(friendDto.getConnection(), friendDto.getUser(), 3, 0));
    ...
    private String writeChat(FriendDto friendDto) throws Exception {
        ...
        chatService.insertFriendMessage(friendDto.getConnection(), friendDto.getUser(), friendDto.getMessage(),
                friendDto.getUser());
        WebSocketHomeServer.sendMessage(Arrays.asList(friendDto.getConnection()), String.format(
                "{\"connectionType\":\"%s\", \"connection\":\"%s\", \"sender\":\"%s\", \"message\":\"%s\"," +
                        "\"type\":\"chat-message\"}", "friend", friendDto.getUser(), friendDto.getUser(),
                friendDto.getMessage()), friendDto.getUser());
        return "success";
    }
    public String updateCalling(FriendDto friendDto) throws Exception {
        friendsService.updateCalling(friendDto.getUser(), friendDto.getConnection(), friendDto.getCalling());
        logger.info("{} {}", friendDto.getUser(), WebSocketHomeServer.hasWsSession(friendDto.getUser()));
        if(friendDto.getCalling() == 1) {
            WebSocketHomeServer.sendMessage(Arrays.asList(friendDto.getUser()), Encoders.getObjectEncoded(new RtcpcDto(
                    "rtcpc", "notify-peer", String.format("{\"peerType\":\"%s\", \"state\":\"ring\"}",
                    "friend"), friendDto.getUser(), friendDto.getConnection())), friendDto.getConnection());
        } else {
            WebSocketHomeServer.sendMessage(Arrays.asList(friendDto.getUser()), Encoders.getObjectEncoded(new RtcpcDto(
                    "rtcpc", "notify-peer", String.format("{\"peerType\":\"%s\", \"state\":\"stopRing\"}",
                    "friend"), friendDto.getUser(), friendDto.getConnection())), friendDto.getConnection());
        }
